...
"""
Run CN LLM-only extraction baseline on existing workbook raw candidates.
...
from llm_cn_verifier import (
    ...
    FIXED_TEMPERATURE,
    REQUEST_TIMEOUT,
    build_cases_from_workbook,
    find_cn_workbooks,
)


SYSTEM_PROMPT = """You are a strict extractor for CN annual-report financial fields.
...
def apply_llm_only_guardrails(
    ...
    for e in evidence:
        ...
        if st == "NOT_APPLICABLE" or ("不适用" in raw):
            supports_na = True
...
def run_llm_only_on_workbook(
    ...
    *,
    ...
    cases = build_cases_from_workbook(workbook)
    if max_fields > 0:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run CN LLM-only extraction baseline.")
    ...
    p.add_argument("--out-csv", default="eval/outputs/llm_only_cn.csv", help="Output CSV path")
