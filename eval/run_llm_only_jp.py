...
"""
Run JP LLM-only extraction baseline on existing workbook candidates.
...
import eval.llm_jp_verifier as jp_verifier  # noqa: E402

DEFAULT_MODEL = "gpt-4o-2024-11-20"

SYSTEM_PROMPT = """You are a strict extractor for JP annual-report financial fields.
...
def run_llm_only_on_workbook(
    ...
    *,
    ...
    cases = jp_verifier.core.build_cases_from_workbook(workbook)
    if max_fields > 0:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run JP LLM-only extraction baseline.")
    ...
    p.add_argument("--out-csv", default="eval/outputs/llm_only_jp.csv", help="Output CSV path")
...
def main() -> None:
    ...
    if args.workbook:
        ...
        workbooks = jp_verifier.find_jp_workbooks(Path(args.input_dir), pattern=args.pattern)
    if not workbooks:
